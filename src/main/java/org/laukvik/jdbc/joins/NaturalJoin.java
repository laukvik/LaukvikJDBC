...
/**
 * A natural join offers a further specialization of quie joins.
 * The join predicate arises implicity by comparing all columns
 * in both tables that have the same column-name in the joined tables.
 * The resulting joined table contains only one column for each pair 
...
public class NaturalJoin extends Join {
	...
	public ColumnData join( ColumnData lt, ColumnData rt ) {
		...
		Vector<Column> sharedLeft = new Vector<Column>();
		Vector<Column> sharedRight = new Vector<Column>();
